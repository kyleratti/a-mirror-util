 ...
 */
const stringOrNull = (varName) => process.env[varName] ? String(process.env[varName]) : null;
/**
 * Retrieve a number value of the specified environment variable, or null if it isn't set
 * @param varName The name of the environment variable to read from `process.env`
 * @example numberOrNull("WEB_PORT")
 */
const numberOrNull = (varName) => process.env[varName] ? Number(process.env[varName]) : null;
exports.default = {
    ...
    reddit: {
        ...
        scanSubsList: (_a = stringOrNull(process.env.REDDIT_SCAN_SUBS)) === null || _a === void 0 ? void 0 : _a.split(","),
    },
    /** Storage configuration */
    storage: {
